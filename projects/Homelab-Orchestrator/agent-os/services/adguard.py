...
class AdGuardService(BaseAgentService):
    """AdGuard Home Management Service"""

    def __init__(self, config: dict[str, Any]):
        super().__init__(config)
        self.base_url = self.config.get("adguard_url", "http://adguard.local:3000")
        self.username = self.config.get("adguard_username", "admin")
        self.password = self.config.get("adguard_password", "your_password")
    ...
    async def initialize(self):
        ...
        try:
            ...
            await self._login()
            self.logger.info("AdGuard service initialized successfully")

        except Exception as e:
            self.logger.error(f"AdGuard initialization error: {e}")
            raise
    ...
    async def _login(self):
        ...
        try:
            ...
            self.logger.error(f"AdGuard login error: {e}")
            raise
    ...
    async def run(self) -> None:
        """Run the main logic of the service. For AdGuardService, this is a no-op as it's primarily reactive."""
        self.logger.info("AdGuard service is running in reactive mode.")
        while True:
            await asyncio.sleep(self.config.get("check_interval", 60)) # Keep alive and allow for checks
            # No active tasks for AdGuardService, it responds to requests
            pass
    ...
    async def get_querylog(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting querylog: {e}")
            raise
    ...
    async def enable_filter(self, filter_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error enabling filter: {e}")
            raise
    ...
    async def disable_filter(self, filter_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error disabling filter: {e}")
            raise
    ...
    async def add_whitelist_rule(self, rule: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error adding whitelist rule: {e}")
            raise
    ...
    async def remove_whitelist_rule(self, rule: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error removing whitelist rule: {e}")
            raise
    ...
    async def rewrite_dns(self, domain: str, answer: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error adding DNS rewrite: {e}")
            raise
    ...
    async def set_protection(self, enabled: bool) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error setting protection: {e}")
            raise
    ...
    async def cleanup(self):
        ...
        self.logger.info("AdGuard service cleaned up")
