...
class AgentManager:
    ...
    async def _run_service_agent(self, agent: Agent):
        """Run a service agent"""
        self.logger.info(f"Starting service agent: {agent.name} (Type: {agent.type.value})")

        service_instance = None
        try:
            if agent.name not in self.service_classes:
                raise ValueError(f"Unknown service agent name: {agent.name}")

            service_class = self.service_classes[agent.name]
            service_instance = service_class(agent.config)

            await service_instance.initialize()
            self.registry.update_agent_status(agent.id, AgentStatus.RUNNING)

            # Run the service's main logic
            ...
            self.registry.update_agent_status(agent.id, AgentStatus.STOPPED)
